...
namespace SP18.PF.Core.Features.Shared
{
    public class States
    {
        public static readonly IList<State> List;

        public static bool IsStateCode(string value)
        {
            return List.Any(x => string.Equals(x.Abbr, value, StringComparison.InvariantCultureIgnoreCase));
        }

        static States()
        {
            var states = new List<State>(50)
            {
            ...
            List = new ReadOnlyCollection<State>(states);
        }
    }
}